/** Dependency-free canonicalization + SHA-256 for portable/offline runtimes. */
...
const add = (...xs: number[]) => xs.reduce((a, b) => (a + b) >>> 0, 0);
...
export function stableId(prefix: string, input: unknown, length = 16): string {
  return `${prefix}-${sha256(canonicalJson(input)).slice(0, length)}`;
}
